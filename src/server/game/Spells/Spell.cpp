...
void Spell::SelectImplicitTrajTargets(SpellEffIndex effIndex)
{
    ...
    std::list<WorldObject*> targets;
    Trinity::WorldObjectSpellTrajTargetCheck check(dist2d, m_targets.GetDstPos(), m_caster, m_spellInfo); 
    Trinity::WorldObjectListSearcher<Trinity::WorldObjectSpellTrajTargetCheck> searcher(m_caster, targets, check, GRID_MAP_TYPE_MASK_ALL);
    SearchTargets<Trinity::WorldObjectListSearcher<Trinity::WorldObjectSpellTrajTargetCheck> > (searcher, GRID_MAP_TYPE_MASK_ALL, m_caster, m_targets.GetDstPos(), dist2d); 
    if (targets.empty())
        return;
...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    if (result != SPELL_CAST_OK && !IsAutoRepeat() && m_spellInfo->Id != 43658)          //always cast autorepeat dummy for triggering
    {
        ...
        if (m_caster->GetTypeId() == TYPEID_PLAYER)  // imported from trinity
        {
...
void Spell::update(uint32 difftime)
{
        ...
        !m_caster->HasAuraTypeWithAffectMask(SPELL_AURA_CAST_WHILE_WALKING, m_spellInfo))
    {
        ...
        if (!IsNextMeleeSwingSpell() && !IsAutoRepeat() && !IsTriggered() && !m_caster->CanCastWhileWalking(m_spellInfo->Id) && m_spellInfo->IsMoveAllowedChannel())
        {
            // if charmed by creature, trust the AI not to cheat and allow the cast to proceed
            // @todo this is a hack, "creature" movesplines don't differentiate turning/moving right now
            // however, checking what type of movement the spline is for every single spline would be really expensive
            if (!IS_CRE_OR_VEH_OR_PET_GUID(m_caster->GetCharmerGUID()))
                cancel();
        }
    }
        ...
        switch (lockInfo->Type[j])
        {
            ...
            case LOCK_KEY_SKILL:
            {
                ...
                if (skillId != SKILL_NONE)
                {
                    ...
                    // add the damage modifier from the spell cast (cheat lock / skeleton key etc.)
