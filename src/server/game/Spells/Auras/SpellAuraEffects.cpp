...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleAuraModWeaponCritPercent,                  // 52 SPELL_AURA_MOD_CRIT_PERCENT
    ...
    &AuraEffect::HandleUnused,                                    //178 SPELL_AURA_MOD_DEBUFF_RESISTANCE          implemented in Unit::MagicSpellHitResult
    ...
    &AuraEffect::HandleNULL,                                      //321 SPELL_AURA_321 // Hex - see 93958.
    &AuraEffect::HandleInterfereTarget,                           //322 SPELL_AURA_INTERFERE_TARGETTING
    &AuraEffect::HandleNULL,                                      //323 SPELL_AURA_323 No spells 4.3.4
    &AuraEffect::HandleNULL,                                      //324 SPELL_AURA_324 // Increases spell crit - see 54675.
    &AuraEffect::HandleNULL,                                      //325 SPELL_AURA_325 No spells 4.3.4
    &AuraEffect::HandleNULL,                                      //326 SPELL_AURA_326 // stealth detection - see 58045.
    ...
    &AuraEffect::HandleNULL,                                      //334 SPELL_AURA_334 // Blind targets (84010 just).
    &AuraEffect::HandleNULL,                                      //335 SPELL_AURA_335 // Invisibility related.
    ...
    &AuraEffect::HandleNULL,                                      //341 SPELL_AURA_341 // Modifies cooldown of all spells using affected category. - ex.: Archaeology (92682).
    ...
    switch (GetAuraType())
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE:
            ...
            else if (GetSpellInfo()->SpellFamilyName == SPELLFAMILY_WARRIOR && GetSpellInfo()->SpellFamilyFlags[0] & 0x20)
            {
                ...
                int32 mws = caster->GetAttackTime(BASE_ATTACK) / 1000;
                float mwb_min = caster->GetWeaponDamageRange(BASE_ATTACK, MINDAMAGE);
                float mwb_max = caster->GetWeaponDamageRange(BASE_ATTACK, MAXDAMAGE);
                float mwb = ((mwb_min + mwb_max) / 2 + ap * mws / 14) * 0.25f;
        ...
        case SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN:
            if (!caster)
                break;
            // Hand of Salvation
            if (GetSpellInfo()->SpellFamilyName == SPELLFAMILY_PALADIN && GetSpellInfo()->SpellFamilyFlags[0] & 0x00000100)
            {
                //Glyph of Salvation
                if (caster->GetGUID() == GetBase()->GetUnitOwner()->GetGUID())
...
void AuraEffect::HandleAuraModIncreaseSpeed(AuraApplication const* aurApp, uint8 mode, bool /*apply*/) const
{
    ...
    target->UpdateSpeed(MOVE_RUN, target->GetTypeId() == TYPEID_PLAYER && target == aurApp->GetBase()->GetCaster() ? false : true);
}
...
void AuraEffect::HandleAuraModIncreaseSwimSpeed(AuraApplication const* aurApp, uint8 mode, bool /*apply*/) const
{
    ...
    target->UpdateSpeed(MOVE_SWIM, target->GetTypeId() == TYPEID_PLAYER && target == aurApp->GetBase()->GetCaster() ? false : true);
}
...
void AuraEffect::HandleAuraModDecreaseSpeed(AuraApplication const* aurApp, uint8 mode, bool /*apply*/) const
{
    ...
    if(target->GetTypeId() == TYPEID_PLAYER && target == aurApp->GetBase()->GetCaster())
        selfCasted = true;
    else
        selfCasted = false;

    target->UpdateSpeed(MOVE_RUN, !selfCasted);
    target->UpdateSpeed(MOVE_SWIM, !selfCasted);
    target->UpdateSpeed(MOVE_FLIGHT, !selfCasted);
    target->UpdateSpeed(MOVE_RUN_BACK, !selfCasted);
    target->UpdateSpeed(MOVE_SWIM_BACK, !selfCasted);
    target->UpdateSpeed(MOVE_FLIGHT_BACK, !selfCasted);
}
...
void AuraEffect::HandleModStateImmunityMask(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    switch (miscVal)
    {
        ...
        case 1887 :
        {
            ...
            {
                mechanic_immunity_list = (1 << MECHANIC_SNARE) | (1 << MECHANIC_ROOT)
                    | (1 << MECHANIC_POLYMORPH) | (1 << MECHANIC_TURN)
                    | (1 << MECHANIC_CHARM) | (1 << MECHANIC_HORROR)
                    | (1 << MECHANIC_HORROR) | (1 << MECHANIC_BLEED)
                    | (1 << MECHANIC_FEAR) | (1<<MECHANIC_KNOCKOUT);
...
void AuraEffect::HandleAuraModIncreaseHealth(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    switch (GetId())
    {
        case 22842: // Frenzied Regeneration
            if (apply)
            {
                target->SetMaxHealth(target->GetMaxHealth() + (target->GetMaxHealth() * 0.3f));

                if (target->GetHealth() < (target->GetMaxHealth() * 0.3f))
                    target->SetHealth(target->GetHealth() + (target->GetHealth() * 0.3f));
            }
            else
...
void AuraEffect::HandleModMeleeSpeedPct(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    int32 miscA = aurApp->GetBase()->GetSpellInfo()->Effects[GetEffIndex()].MiscValue;
...
void AuraEffect::HandleAuraDummy(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (mode & (AURA_EFFECT_HANDLE_REAL | AURA_EFFECT_HANDLE_REAPPLY))
    {
        ...
        else
        {
            ...
            case SPELLFAMILY_PRIEST:
                // Vampiric Touch
                if (GetSpellInfo()->SpellFamilyFlags[1] & 0x0400 && aurApp->GetRemoveMode() == AURA_REMOVE_BY_ENEMY_SPELL && GetEffIndex() == 0)
                {
                    // Sin and Punishment
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            switch (GetId())
            {
                ...
                {
                    if (apply)
                        if (target->HasAura(48539) || target->HasAura(48544))
                            target->CastSpell(target, 57669, true);
                break;
                }
            }
        }
...
void AuraEffect::HandlePeriodicDummyAuraTick(Unit* target, Unit* caster) const
{
    switch (GetSpellInfo()->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_GENERIC:
            switch (GetId())
            {
                ...
                case 62399: // Overload Circuit
                    ...
                    break;
                case 75592: // Searing Light Anhuur
                case 94949: // Searing Light Anhuur
                caster->CastSpell(target, 75591, true);
                break;
        ...
        case SPELLFAMILY_DRUID:
        {
            switch (GetSpellInfo()->Id)
            {
                ...
                case 22842:
                {
                    ...
                    int32 points = target->CalculateSpellDamage(target, GetSpellInfo(), 0);
                    int32 regen = (target->GetMaxHealth() * (mod * points / 10) / 1000) / 210;
                    target->CastCustomSpell(target, 22845, &regen, 0, 0, true, 0, this);
                    if (!target->HasAura(54810))                                           // Glyph of Frenzied Regeneration
        ...
        case SPELLFAMILY_SHAMAN:
            ...
            {
                if (DynamicObject* dynObj = caster->GetDynObject(GetId()))
                    caster->CastSpell(dynObj->GetPositionX(),dynObj->GetPositionY(),dynObj->GetPositionZ(), 73921, true);
                break;
            }
            break;
        case SPELLFAMILY_DEATHKNIGHT:
            ...
            // Blood Rites
            // Death Rune Mastery
            if (GetSpellInfo()->SpellIconID == 22 || GetSpellInfo()->SpellIconID == 2622 || GetSpellInfo()->SpellIconID == 2724)
            {
                if (target->GetTypeId() != TYPEID_PLAYER)
                    return;
...
void AuraEffect::HandlePeriodicTriggerSpellAuraTick(Unit* target, Unit* caster) const
{
                ...
                switch (auraId)
                {
                    ...
                    case 83676:
                        if (target->isMoving() && caster)
                            if (AuraEffect* aurEff = caster->GetDummyAuraEffect(SPELLFAMILY_HUNTER, 5119, 0))
        ...
        switch (auraId)
        {
            ...
            case 6474:
            {
                if (Unit *owner = caster->GetOwner())
                    if (AuraEffect const* aurEff = owner->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_SHAMAN, 2289, 0))
                        if (roll_chance_i(aurEff->GetAmount()))
    ...
    if (auraId == 81781 && GetEffIndex() == 1)
        if (caster->GetOwner() && !caster->GetOwner()->HasAura(55689))
            return;
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
    {
        switch (GetSpellInfo()->Id)
        {
            case 43093: case 31956: case 38801: case 97639: // Grievous Wound
            ...
            case 15407:
                ...
                {
                    int chance = 10;
                    if (caster->HasAura(33191)) chance += 4;
                    else if (caster->HasAura(78228)) chance += 8;
    ...
    if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
    {
        ...
        // Censure
        if (GetSpellInfo()->Id == 31803)
        {
            // damage = (0.01 * SPH + 0.0270 * AP) * 5 / total ticks * stacks on target
            damage = (0.01 * (caster->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_HOLY) + target->SpellBaseDamageBonusTaken(SPELL_SCHOOL_MASK_HOLY)) + 0.027 * caster->GetTotalAttackPowerValue(BASE_ATTACK)) * 5 / GetTotalTicks() * GetBase()->GetStackAmount();
        }
        // Frost Fever + Blood Plague
        if (GetSpellInfo()->Id == 55095 || GetSpellInfo()->Id == 55078)
        {
            damage = damage * 1.15 + (caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.21) * 1.15;
        }
        ...
        if (GetSpellInfo()->SpellFamilyName == SPELLFAMILY_GENERIC)
        {
            switch (GetId())
            {
                ...
                case 72856: // Unbound Plague
                    ...
                    break;
                case 88954:  // Consuming Darkness Argaloth
                case 95173:
                    damage *= uint32(pow(1.10f, int32(m_tickNumber)));
                    break;
                case 82665:  // Heart Of Ice
                    damage *= uint32(pow(1.10f, int32(m_tickNumber)));
                    break;
                case 89421:  // Wreck Sinestra
                case 89435:
                case 92955:
                case 92956:
                    damage *= uint32(pow(1.5f, int32(m_tickNumber)));
                    break;
                default:
                    break;
            }
