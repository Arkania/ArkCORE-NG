...
void GuildMgr::LoadGuilds()
{
    ...
    sLog->outString("Loading challenges...");
    {
        for (GuildContainer::const_iterator itr = GuildStore.begin(); itr != GuildStore.end(); ++itr)
        {
...
void GuildMgr::LoadGuildXpForLevel()
{
    ...
    if (!result)
    {
        sLog->outString(">> Loaded 0 xp for guild level definitions. DB table `guild_xp_for_level` is empty.");
        sLog->outString();
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->outString("Unused (> Guild.MaxLevel in worldserver.conf) level %u in `guild_xp_for_level` table, ignoring.", uint32(level));
            continue;
        }
