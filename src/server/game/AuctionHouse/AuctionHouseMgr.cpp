...
bool AuctionEntry::BuildAuctionInfo(WorldPacket& data) const
{
    ...
    if (!item)
    {
        sLog->outError("AuctionEntry::BuildAuctionInfo: Auction %u has a non-existent item: %u", Id, itemGUIDLow);
        return false;
    }
    data << uint32(Id);
...
void AuctionEntry::SaveToDB(SQLTransaction& trans) const
{
    ...
    stmt->setInt32 (7, bid);
    stmt->setInt32 (8, startbid);
    stmt->setInt32 (9, deposit);
    trans->Append(stmt);
}
...
bool AuctionEntry::LoadFromDB(Field* fields)
{
    ...
    {
        sLog->outError("Auction %u has not a existing item : %u", Id, itemGUIDLow);
        return false;
    }
    return true;
}
