...
WorldObject::~WorldObject()
{
    ...
    if (IsWorldObject() && m_currMap)
    {
        ...
        {
            sLog->outCrash("Object::~Object Corpse guid="UI64FMTD", type=%d, entry=%u deleted but still in map!!",
...
void Object::_BuildMovementUpdate(ByteBuffer* data, uint16 flags) const
{
    ...
    data->WriteBit(0);
    data->WriteBit(flags & (UPDATEFLAG_TRANSPORT | UPDATEFLAG_TRANSPORT_ARR));
    ...
    else if (isType(TYPEMASK_GAMEOBJECT))                    // gameobject case
    {
        for (uint16 index = 0; index < valCount; ++index)
        {
            if (updateMask->GetBit(index))
            {
                ...
                {
                    if (ToGameObject()->GetGoType() == GAMEOBJECT_TYPE_TRANSPORT)
...
bool Position::HasInLine(WorldObject const* target, float width) const
{
    ...
    return fabs(std::sin(angle)) * GetExactDist2d(target->GetPositionX(), target->GetPositionY()) < width;
}
...
Pet* Player::SummonPet(uint32 entry, float x, float y, float z, float ang, PetType petType, uint32 duration, PetSaveMode slotID)
{
    ...
    SetMinion(pet, true, petType == HUNTER_PET ? PET_SLOT_HUNTER_FIRST : PET_SLOT_OTHER_PET);
    ...
    switch (petType)
    {
        case SUMMON_PET:
            ...
            pet->SavePetToDB(PET_SLOT_HUNTER_FIRST);
            PetSpellInitialize();
            break;
        default:
            break;
