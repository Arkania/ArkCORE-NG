...
void Transport::CalculatePassengerOffset(float& x, float& y, float& z, float& o)
{
    ...
    float inx = x, iny = y;
    y = (iny - inx * tan(GetOrientation())) / (std::cos(GetOrientation()) + std::sin(GetOrientation()) * tan(GetOrientation()));
    x = (inx + iny * tan(GetOrientation())) / (std::cos(GetOrientation()) + std::sin(GetOrientation()) * tan(GetOrientation()));
}
