...
#define PLAYER_EXPLORED_ZONES_SIZE  156 //Last found in 4.3.4.
...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        Pet* GetPet() const;
        Pet* SummonPet(uint32 entry, float x, float y, float z, float ang, PetType petType, uint32 despwtime, PetSaveMode slotID);
        ...
        void RemoveActiveQuest(uint32 quest_id, bool update = true);
        void RemoveRewardedQuest(uint32 quest_id, bool update = true);
        void SendQuestUpdate(uint32 quest_id);
        ...
        bool HasMastery();
        float GetMasteryPoints() { return CalculateMasteryFromMasteryRating(m_baseRatingValue[CR_MASTERY]); }
        float CalculateMasteryFromMasteryRating(int32 curr_rating)  { return float(curr_rating * 0.0055779569892473); }
        int32 CalculateMasteryRatingFromMastery(float curr_mastery) { return int32(curr_mastery / 0.0055779569892473); }

        void ArmorSpecialization();
        ...
        PetSaveMode getSlotForNewPet()
        {
            ...
            int32 slot3 = 0;
            int32 slot4 = 0;
            if (QueryResult Slots = CharacterDatabase.PQuery("SELECT slot FROM character_pet WHERE owner ='%u'", GetGUID()))
            {
                do
                {
                    int32 HunterSlot = (*Slots)[0].GetInt32();
                    if (HunterSlot == 0)
                        slot0 = 1;
                    else if (HunterSlot == 1)
                        slot1 = 2;
                    else if (HunterSlot == 2)
                        slot2 = 3;
                    else if (HunterSlot == 3)
                        slot3 = 4;
                    else if (HunterSlot == 4)
                        slot4 = 5;
                }
                while (Slots->NextRow());
            }
        ...
        bool isInWorgenForm();
        void setInHumanForm();
        void setInWorgenForm(uint32 form = UNIT_FLAG2_WORGEN_TRANSFORM2);
        bool toggleWorgenForm(uint32 form = UNIT_FLAG2_WORGEN_TRANSFORM2);
    ...
    protected:
        ...
        */
        ...
        void _LoadCurrencyPoints(PreparedQueryResult result);
        ...
        void _SaveCurrencyPoints(SQLTransaction& trans);
