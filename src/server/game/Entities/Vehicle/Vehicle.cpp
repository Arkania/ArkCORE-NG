...
bool Vehicle::AddPassenger(Unit* unit, int8 seatId)
{
    ...
    if (_me->IsInWorld())
    {
        ...
        if (Player* player = unit->ToPlayer())
            player->SendAutoRepeatCancel(unit);
...
void Vehicle::CalculatePassengerOffset(float& x, float& y, float& z, float& o)
{
    ...
    float inx = x, iny = y;
    y = (iny - inx * tan(GetBase()->GetOrientation())) / (std::cos(GetBase()->GetOrientation()) + std::sin(GetBase()->GetOrientation()) * tan(GetBase()->GetOrientation()));
    x = (inx + iny * tan(GetBase()->GetOrientation())) / (std::cos(GetBase()->GetOrientation()) + std::sin(GetBase()->GetOrientation()) * tan(GetBase()->GetOrientation()));
}