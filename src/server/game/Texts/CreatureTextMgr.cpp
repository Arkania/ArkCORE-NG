    ...
    public:
        ...
        size_t operator()(WorldPacket* data, LocaleConstant locale) const
        {
            ...
            *data << uint8(0);                                       // ChatTag
            if (_msgType == CHAT_MSG_RAID_BOSS_WHISPER || _msgType == CHAT_MSG_RAID_BOSS_EMOTE)
            {
...
class PlayerTextBuilder
{
    public:
        ...
        size_t operator()(WorldPacket* data, LocaleConstant locale) const
        {
            ...
            *data << uint32(0); // 2.1.0
            *data << uint32(strlen(_talker->GetName()) + 1);
...
void CreatureTextMgr::LoadCreatureTextLocales()
{
    ...
    {
        Field* fields = result->Fetch();
        CreatureTextLocale& loc = mLocaleTextMap[CreatureTextId(fields[0].GetUInt32(), uint32(fields[1].GetUInt8()), fields[2].GetUInt8())];
