...
void ArenaTeam::DelMember(uint64 guid, bool cleanDb)
{
    ...
    if (Player* player = ObjectAccessor::FindPlayer(guid))
    {
        ...
        sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Player: %s [GUID: %u] left arena team type: %u [Id: %u].", player->GetName().c_str(), player->GetGUIDLow(), GetType(), GetId());
    }
...
void ArenaTeam::Disband(WorldSession* session)
{
    ...
    if (session)
    {
        ...
        if (Player* player = session->GetPlayer())
            sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Player: %s [GUID: %u] disbanded arena team type: %u [Id: %u].", player->GetName().c_str(), player->GetGUIDLow(), GetType(), GetId());
    }
