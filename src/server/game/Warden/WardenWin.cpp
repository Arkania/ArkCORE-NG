...
void WardenWin::HandleHashResult(ByteBuffer &buff)
{
    ...
    {
        sLog->outWarden("WARDEN: %s failed hash reply. Action: %s", _session->GetPlayerInfo().c_str(), Penalty().c_str());
        return;
    }
...
void WardenWin::HandleData(ByteBuffer &buff)
{
    ...
    {
        buff.rpos(buff.wpos());
        sLog->outWarden("WARDEN: %s failed checksum. Action: %s", _session->GetPlayerInfo().c_str(), Penalty().c_str());
        return;
    }
    ...
    // TIMING_CHECK
    {
        ...
        if (result == 0x00)
        {
            sLog->outWarden("WARDEN: %s failed timing check. Action: %s", _session->GetPlayerInfo().c_str(), Penalty().c_str());
            return;
        }
    ...
    if (checkFailed > 0)
    {
        ...
        sLog->outWarden("WARDEN: %s failed Warden check %u. Action: %s", _session->GetPlayerInfo().c_str(), checkFailed, Penalty(check).c_str());
    }
