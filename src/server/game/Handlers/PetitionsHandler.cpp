...
void WorldSession::HandlePetitionSignOpcode(WorldPacket & recvData)
{
    ...
    uint32 signs = fields[1].GetUInt32();
    ...
    else
    {
        if (_player->GetGuildId())
        {
            Guild::SendCommandResult(this, GUILD_COMMAND_INVITE, ERR_ALREADY_IN_GUILD_S, _player->GetName().c_str());
            return;
        }
        if (_player->GetGuildIdInvited())
        {
            Guild::SendCommandResult(this, GUILD_COMMAND_INVITE, ERR_ALREADY_INVITED_TO_GUILD_S, _player->GetName().c_str());
            return;
        }
    }
