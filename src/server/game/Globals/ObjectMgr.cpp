    ...
    do
    {
        ...
        switch (summonerType)
        {
            case SUMMONER_TYPE_CREATURE:
                ...
                {
                    sLog->outErrorDb("Table `creature_summon_groups` has summoner with non existing entry %u for creature summoner type, skipped.", summonerId);
                    continue;
                }
                break;
            case SUMMONER_TYPE_GAMEOBJECT:
                ...
                {
                    sLog->outErrorDb("Table `creature_summon_groups` has summoner with non existing entry %u for gameobject summoner type, skipped.", summonerId);
                    continue;
                }
                break;
            case SUMMONER_TYPE_MAP:
                ...
                {
                    sLog->outErrorDb("Table `creature_summon_groups` has summoner with non existing entry %u for map summoner type, skipped.", summonerId);
                    continue;
                }
                break;
            default:
                sLog->outErrorDb("Table `creature_summon_groups` has unhandled summoner type %u for summoner %u, skipped.", summonerType, summonerId);
                continue;
        }
        ...
        {
            sLog->outErrorDb("Table `creature_summon_groups` has creature in group [Summoner ID: %u, Summoner Type: %u, Group ID: %u] with non existing creature entry %u, skipped.", summonerId, summonerType, group, data.entry);
            continue;
        }
        ...
        {
            sLog->outErrorDb("Table `creature_summon_groups` has unhandled temp summon type %u in group [Summoner ID: %u, Summoner Type: %u, Group ID: %u] for creature entry %u, skipped.", data.type, summonerId, summonerType, group, data.entry);
            continue;
        }
...
void ObjectMgr::LoadCreatures()
{
    ...
    do
    {
        ...
        // Map 661 was removed from dbc as of 4.x.x
        spawnMasks[661] = 1;
        ...
        else if (data.movementType == IDLE_MOTION_TYPE)
        {
            if (data.spawndist != 0.0f)
            {
                sLog->outErrorDb("Table `creature` have creature (GUID: %u Entry: %u) with `MovementType`=0 (idle) have `spawndist`<>0, MovementType set to 1.", guid, data.id);
                data.movementType = RANDOM_MOTION_TYPE;
            }
        }
...
void ObjectMgr::LoadItemTemplates()
{
    ...
    if (result)
    {
        do
        {
            ...
            itemTemplate.Armor   = FillItemArmor(itemTemplate.ItemLevel, itemTemplate.Class, itemTemplate.SubClass, itemTemplate.Quality,
    ...
    sLog->outString("Loading Player Create Spell Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    sLog->outError("Wrong race: %u, class: %u combination in `playercreateinfo_spell` table, ignoring.", current_race, current_class);
                    continue;
                }
...
void ObjectMgr::LoadQuests()
{
    ...
    for (QuestMap::iterator iter = _questTemplates.begin(); iter != _questTemplates.end(); ++iter)
    {
        ...
        {
            sLog->outErrorDb("Quest %u should be disabled because `MinLevel` = %i", qinfo->GetQuestId(), int32(qinfo->MinLevel));
            ...
            if (id)
            {
                ...
                if (!qinfo->RequiredNpcOrGo[j])
                {
                    ...
                    if (found)
                    {
                        ...
                        {
                            sLog->outErrorDb("Spell (id: %u) have SPELL_EFFECT_QUEST_COMPLETE or SPELL_EFFECT_SEND_EVENT for quest %u and RequiredNpcOrGo%d = 0, but quest not have flag QUEST_SPECIAL_FLAGS_EXPLORATION_OR_EVENT. Quest flags or RequiredNpcOrGo%d must be fixed, quest modified to enable objective.", spellInfo->Id, qinfo->Id, j+1, j+1);
...
void ObjectMgr::LoadReputationRewardRate()
{
    ...
    do
    {
        ...
        {
            sLog->outErrorDb("Table reputation_reward_rate has creature_rate with invalid rate %f, skipping data for faction %u", repRate.creatureRate, factionId);
            continue;
        }

        if (repRate.spellRate < 0.0f)
        {
            sLog->outErrorDb("Table reputation_reward_rate has spell_rate with invalid rate %f, skipping data for faction %u", repRate.spellRate, factionId);
            continue;
        }
...
void ObjectMgr::LoadReputationOnKill()
{
    ...
    do
    {
        ...
        if (repOnKill.currencyid1)
        {
            ...
            {
                sLog->outErrorDb("CurrenctType (CurrenctTypes.dbc) %u does not exist but is used in `creature_onkill_reward`", repOnKill.currencyid1);
                continue;
            }
        }
        ...
        if (repOnKill.currencyid2)
        {
            ...
            {
                sLog->outErrorDb("CurrenctType (CurrenctTypes.dbc) %u does not exist but is used in `creature_onkill_reward`", repOnKill.currencyid2);
                continue;
            }
        }
        ...
        if (repOnKill.currencyid3)
        {
            ...
            {
                sLog->outErrorDb("CurrenctType (CurrenctTypes.dbc) %u does not exist but is used in `creature_onkill_reward`", repOnKill.currencyid3);
                continue;
            }
        }
...
bool ObjectMgr::IsVendorItemValid(uint32 vendor_entry, uint32 id, int32 maxcount, uint32 incrtime, uint32 ExtendedCost, uint8 type, Player* player, std::set<uint32>* skip_vendors, uint32 ORnpcflag) const
{
        ...
        (type == ITEM_VENDOR_TYPE_CURRENCY && !sCurrencyTypesStore.LookupEntry(id)))
    {
        if (player)
            ...
            sLog->outErrorDb("Table `(game_event_)npc_vendor` for Vendor (Entry: %u) have in item list non-existed item (%u, type %u), ignore", vendor_entry, id, type);
        return false;
    }
    ...
    if (ExtendedCost && !sItemExtendedCostStore.LookupEntry(ExtendedCost))
    {
        if (player)
            ...
            sLog->outErrorDb("Table `(game_event_)npc_vendor` have Item (Entry: %u) with wrong ExtendedCost (%u) for vendor (%u), ignore", id, ExtendedCost, vendor_entry);
        return false;
    }
    ...
    if (type == ITEM_VENDOR_TYPE_ITEM) // not applicable to currencies
    {
        if (maxcount > 0 && incrtime == 0)
        {
            if (player)
                ...
                sLog->outErrorDb("Table `(game_event_)npc_vendor` has `maxcount` (%u) for item %u of vendor (Entry: %u) but `incrtime`=0, ignore", maxcount, id, vendor_entry);
            return false;
        }
        else if (maxcount == 0 && incrtime > 0)
        {
            if (player)
                ...
                sLog->outErrorDb("Table `(game_event_)npc_vendor` has `maxcount`=0 for item %u of vendor (Entry: %u) but `incrtime`<>0, ignore", id, vendor_entry);
            return false;
        }
    }
    ...
    if (vItems->FindItemCostPair(id, ExtendedCost, type))
    {
        if (player)
            ...
            sLog->outErrorDb("Table `npc_vendor` has duplicate items %u (with extended cost %u, type %u) for vendor (Entry: %u), ignoring", id, ExtendedCost, type, vendor_entry);
        return false;
    }
...
void ObjectMgr::LoadFactionChangeQuests()
{
    ...
    do
    {
        ...
        if (!sObjectMgr->GetQuestTemplate(alliance))
            sLog->outErrorDb("Quest %u (alliance_id) referenced in `player_factionchange_quests` does not exist, pair skipped!", alliance);
        else if (!sObjectMgr->GetQuestTemplate(horde))
            sLog->outErrorDb("Quest %u (horde_id) referenced in `player_factionchange_quests` does not exist, pair skipped!", horde);
        else
...
void ObjectMgr::LoadFactionChangeReputations()
{
    ...
    do
    {
        ...
        if (!sFactionStore.LookupEntry(alliance))
            sLog->outErrorDb("Reputation %u (alliance_id) referenced in `player_factionchange_reputations` does not exist, pair skipped!", alliance);
        else if (!sFactionStore.LookupEntry(horde))
            sLog->outErrorDb("Reputation %u (horde_id) referenced in `player_factionchange_reputations` does not exist, pair skipped!", horde);
        else
...
void ObjectMgr::LoadMissingKeyChains()
{
    ...
    if (!result)
    {
        sLog->outString(">> Loaded 0 KeyChain entries. DB table `keychain_db2` is empty.");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Loaded %u KeyChain entries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void ObjectMgr::LoadFactionChangeTitles()
{
    ...
    do
    {
        ...
        if (!sCharTitlesStore.LookupEntry(alliance))
            sLog->outErrorDb("Title %u (alliance_id) referenced in `player_factionchange_title` does not exist, pair skipped!", alliance);
        else if (!sCharTitlesStore.LookupEntry(horde))
            sLog->outErrorDb("Title %u (horde_id) referenced in `player_factionchange_title` does not exist, pair skipped!", horde);
        else
...
void ObjectMgr::LoadPhaseDefinitions()
{
    ...
    do
    {
        ...
        {
            sLog->outErrorDb("Flags defined in phase_definitions in zoneId %d and entry %u does contain PHASE_FLAG_OVERWRITE_EXISTING and PHASE_FLAG_NEGATE_PHASE. Setting flags to PHASE_FLAG_OVERWRITE_EXISTING", PhaseDefinition.zoneId, PhaseDefinition.entry);
...
void ObjectMgr::LoadSpellPhaseInfo()
{
    ...
    do
    {
        ...
        if (!spell)
        {
            sLog->outErrorDb("Spell %u defined in `spell_phase` does not exists, skipped.", spellPhaseInfo.spellId);
            continue;
        }

        if (!spell->HasAura(SPELL_AURA_PHASE))
        {
            sLog->outErrorDb("Spell %u defined in `spell_phase` does not have aura effect type SPELL_AURA_PHASE, useless value.", spellPhaseInfo.spellId);
            continue;
        }
