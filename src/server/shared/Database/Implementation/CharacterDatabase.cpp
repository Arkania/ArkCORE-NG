...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(CHAR_SEL_PET_SLOTS_DETAIL, "SELECT owner, id, entry, level, name, slot FROM character_pet WHERE owner = ? AND slot >= ? AND slot <= ? ORDER BY slot", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_SEL_CHARACTER_QUESTSTATUSREW, "SELECT quest FROM character_queststatus_rewarded WHERE guid = ? AND active = 1", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(CHAR_UPD_GUILD_NAME, "UPDATE guild SET name = ? WHERE guildid = ?", CONNECTION_ASYNC) // 0: string, 1: uint32
    PREPARE_STATEMENT(CHAR_SEL_GUILD_CHALLENGES_COMPLETED, "SELECT challengeId, dateCompleted FROM guild_challenges_completed WHERE guildId = ?", CONNECTION_SYNCH)
    PREPARE_STATEMENT(CHAR_INS_GUILD_CHALLENGE_DONE, "REPLACE INTO guild_challenges_completed (guildId, challengeId, dateCompleted) VALUES (?, ?, ?)", CONNECTION_ASYNC)
    PREPARE_STATEMENT(CHAR_DEL_GUILD_CHALLENGE_EXPIERED, "DELETE FROM guild_challenges_completed WHERE guildId = ? AND challengeId = ?", CONNECTION_ASYNC)
    ...
    PREPARE_STATEMENT(CHAR_GET_GUILD_TOTAL_REP_VAL, "SELECT total_rep FROM character_guild_reputation WHERE guid = ?", CONNECTION_SYNCH); // 0: uint32
    PREPARE_STATEMENT(CHAR_SET_GUILD_TOTAL_REP_VAL, "UPDATE character_guild_reputation SET total_rep = ? WHERE guid = ?", CONNECTION_ASYNC); // 0,1: uint32
    ...
    PREPARE_STATEMENT(CHAR_SEL_CHAR_PET_BY_ENTRY_AND_SLOT_2, "SELECT id, entry, owner, modelid, level, exp, Reactstate, slot, name, renamed, curhealth, curmana, abdata, savetime, CreatedBySpell, PetType FROM character_pet WHERE owner = ? AND entry = ? AND ((slot >= ? AND slot <= ?) OR slot > ?)", CONNECTION_SYNCH);
    PREPARE_STATEMENT(CHAR_SEL_CHAR_PET_BY_SLOT, "SELECT id, entry, owner, modelid, level, exp, Reactstate, slot, name, renamed, curhealth, curmana, abdata, savetime, CreatedBySpell, PetType FROM character_pet WHERE owner = ? AND ((slot >= ? AND slot <= ?) OR slot > ?)", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(CHAR_INS_CHAR_QUESTSTATUS_REWARDED, "INSERT IGNORE INTO character_queststatus_rewarded (guid, quest, active) VALUES (?, ?, 1)", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_UPD_CHAR_QUESTSTATUS_REWARDED_FACTION_CHANGE, "UPDATE character_queststatus_rewarded SET quest = ? WHERE quest = ? AND guid = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_CHAR_QUESTSTATUS_REWARDED_ACTIVE, "UPDATE character_queststatus_rewarded SET active = 1 WHERE guid = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_UPD_CHAR_QUESTSTATUS_REWARDED_ACTIVE_BY_QUEST, "UPDATE character_queststatus_rewarded SET active = 0 WHERE quest = ? AND guid = ?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(CHAR_SEL_PLAYER_SITES, "SELECT siteId, mapId, areaId, find1PosX, find1PosY, find2PosX, find2PosY, find3PosX, find3PosY FROM character_archaeology_digsites WHERE guid = ? AND siteId = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(CHAR_REP_PLAYER_SITES, "REPLACE INTO character_archaeology_digsites (guid, siteId, mapId, areaId, find1PosX, find1PosY, find2PosX, find2PosY, find3PosX, find3PosY) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_DEL_PLAYER_SITES, "DELETE FROM  character_archaeology_digsites WHERE guid = ? AND siteId = ?", CONNECTION_ASYNC);

    PREPARE_STATEMENT(CHAR_SEL_PLAYER_PROJECT, "SELECT projectEntry, branchId FROM character_archaeology_projects WHERE guid = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(CHAR_INS_PLAYER_PROJECT, "INSERT INTO character_archaeology_projects (guid, branchId, projectEntry) VALUES (?,?,?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_DEL_PLAYER_PROJECT, "DELETE FROM character_archaeology_projects WHERE guid = ? AND projectEntry = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_SEL_PLAYER_COMPLETED_PROJECT, "SELECT projectId FROM character_completed_projects WHERE guid = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(CHAR_INS_PLAYER_COMPLETED_PROJECT, "INSERT INTO character_completed_projects (guid, projectId, completionCount, completionTime) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(CHAR_SEL_PLAYER_COMPLETED_PROJECT_COUNT, "SELECT completionCount FROM character_completed_projects WHERE guid = ? AND projectId = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(CHAR_SEL_PLAYER_COMPLETED_PROJECT_TIME, "SELECT completionTime FROM character_completed_projects WHERE guid = ? AND projectId = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(CHAR_UPD_PLAYER_COMPLETED_PROJECT_COUNT, "UPDATE character_completed_projects SET completionCount = ? WHERE guid = ? AND projectId = ?", CONNECTION_ASYNC);
