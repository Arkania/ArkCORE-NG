...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(WORLD_REPLACE_CREATURE_ADDON_EMOTE, "REPLACE creature_addon (guid,emote) VALUES (?,?)", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(WORLD_SEL_GRAVEYARDS, "SELECT id FROM game_graveyard_zone WHERE faction=? OR faction=0", CONNECTION_ASYNC);
    PREPARE_STATEMENT(WORLD_SEL_DIGSITE_INFO, "SELECT Level, Map, AreaId FROM archaeology_digsites WHERE Entry=?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(WORLD_SEL_DIGSITE_POS, "SELECT FindPosition, PositionX, PositionY FROM archaeology_digsites_positions WHERE Id=?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(WORLD_SEL_RANDOM_DIGSITE, "SELECT Entry FROM archaeology_digsites WHERE Level <= ? AND Map = ?", CONNECTION_SYNCH);
    PREPARE_STATEMENT(WORLD_SEL_GUILD_CHALLENGES, "SELECT challengeId, challengeRewardId, challengeType, challengeEntry, xpReward, goldReward, goldExtraReward FROM guild_challenges JOIN guild_challenges_rewards ON guild_challenges.challengeRewardId = guild_challenges_rewards.rewardId", CONNECTION_SYNCH);
}
