...
enum Spells
{
    ...
    SPELL_EMOTE_SIT_NO_SHEATH           = 91298,
                ...
                switch (summon->GetEntry())
                {
                    ...
                    case NPC_ICE_SPHERE:
                    {
                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 200.0f, true))
                        {
                            ...
                            summon->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_TAUNT, false);
...
class npc_valkyr_shadowguard : public CreatureScript
{
    public:
        ...
        struct npc_valkyr_shadowguardAI : public ScriptedAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                if (me->IsInCombat() && !UpdateVictim()) // Grab player starts from passive attitude.
                    return;
        ...
        class spell_the_lich_king_infest_AuraScript : public AuraScript
        {
            ...
            void OnUpdate(AuraEffect* aurEff)
            {
                ...
                if (GetUnitOwner()->HealthAbovePct(90))
                    aurEff->SetAmount(int32(GetSpellInfo()->Effects[0].BasePoints));
                else
