    ...
    static bool HandleModifyMoneyCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (addmoney < 0)
        {
            ...
            else
            {
                ...
                handler->PSendSysMessage(LANG_YOU_TAKE_MONEY, labs(addmoney), handler->GetNameLink(target).c_str());
                if (handler->needReportToTarget(target))
                    (ChatHandler(target)).PSendSysMessage(LANG_YOURS_MONEY_TAKEN, handler->GetNameLink().c_str(), labs(addmoney));
