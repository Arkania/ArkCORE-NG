                ...
                if (UltimateUsed)
                {
                    if (uiDownTimer <= diff)
                    {
                        ...
                        if (me->GetPower(POWER_ENERGY) == 0)
                        {
                            ...
                            if (Creature* nezir = ObjectAccessor::GetCreature(*me, instance ? instance->GetData64(DATA_NEZIR) : 0))
                                ...
                                if (changed)
                                    if (Creature* rohash = ObjectAccessor::GetCreature(*me, instance ? instance->GetData64(DATA_ROHASH) : 0))
...
public:
    ...
    struct npc_ravenous_creeper_triggerAI : public ScriptedAI
    {
        ...
        {
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_PACIFIED | UNIT_FLAG_DISABLE_MOVE);
            me->SetReactState(REACT_PASSIVE);
        }
                ...
                if (UltimateUsed)
                {
                    if (uiDownTimer <= diff)
                    {
                        ...
                        if (me->GetPower(POWER_ENERGY) == 0)
                        {
                            ...
                            bool changed = false;
                            if (Creature* anshal = ObjectAccessor::GetCreature(*me, instance ? instance->GetData64(DATA_ANSHAL) : 0))
