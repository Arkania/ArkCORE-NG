...
namespace MMAP
{
    ...
    bool MMapManager::loadMapData(uint32 mapId)
    {
        ...
        {
            dtFreeNavMesh(mesh);
            TC_LOG_DEBUG("maps", "MMAP:loadMapData: Failed to initialize dtNavMesh for mmap %03u from file %s", mapId, fileName);
            delete [] fileName;
            return false;
        }
    ...
    bool MMapManager::loadMap(const std::string& /*basePath*/, uint32 mapId, int32 x, int32 y)
    {
        ...
        {
            TC_LOG_DEBUG("maps", "MMAP:loadMap: Bad header in mmap %03u%02i%02i.mmtile", mapId, x, y);
            return false;
        }

        if (fileHeader.mmapVersion != MMAP_VERSION)
        {
            TC_LOG_DEBUG("maps", "MMAP:loadMap: %03u%02i%02i.mmtile was built with generator v%i, expected v%i",
        ...
        if (!result)
        {
            TC_LOG_DEBUG("maps", "MMAP:loadMap: Bad header or data in mmap %03u%02i%02i.mmtile", mapId, x, y);
            fclose(file);
            return false;
        }
        ...
        {
            TC_LOG_DEBUG("maps", "MMAP:loadMap: Could not load %03u%02i%02i.mmtile into navmesh", mapId, x, y);
            dtFree(data);
            return false;
        }
    ...
    bool MMapManager::unloadMap(uint32 mapId, int32 x, int32 y)
    {
        ...
        if (dtStatusFailed(mmap->navMesh->removeTile(tileRef, NULL, NULL))) 
        {
            ...
            TC_LOG_DEBUG("maps", "MMAP:unloadMap: Could not unload %03u%02i%02i.mmtile from navmesh", mapId, x, y);
            ASSERT(false);
        }
        else
        {
    ...
    dtNavMeshQuery const* MMapManager::GetNavMeshQuery(uint32 mapId, uint32 instanceId)
    {
        ...
        if (mmap->navMeshQueries.find(instanceId) == mmap->navMeshQueries.end())
        {
            ...
            {
                dtFreeNavMeshQuery(query);
                TC_LOG_DEBUG("maps", "MMAP:GetNavMeshQuery: Failed to initialize dtNavMeshQuery for mapId %03u instanceId %u", mapId, instanceId);
                return NULL;
            }
